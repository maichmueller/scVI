...
class MouseAtlas(DownloadableDataset):
    def __init__(
        ...
        if fpaths_and_fnames is None:
            ...
            super().__init__(
                save_path=save_path, urls=None, filenames=None, delayed_populating=True
            )
            ...
            for ftype, fpath in fpaths_and_fnames.items():
                ...
                if path != self.save_path:
                    if ftype == "cell":
                        ...
                        raise ValueError(
                            f"Key '{ftype}' not useful. Try one of {keys}."
                        )

            filenames = [
                "cleaned_data_sparse.npz",
                "gene_annotation.csv",
                "cell_annotation.csv",
        ...
        if not delayed_population:
            if not os.path.exists(
                os.path.join(self.save_path, "cleaned_data_sparse.npz")
            ):
    ...
    def populate(self):
        ...
        gene_annotation = self._read_annotation_file(
            os.path.join(self.save_path, self.filenames[1])
        )
        if self.use_ensembl:
            gene_names = np.array(
                [gn.split(".")[0] for gn in gene_annotation["gene_id"]]
            )
        else:
            gene_names = gene_annotation["gene_short_name"]
        cell_annotation = self._read_annotation_file(
            os.path.join(self.save_path, self.filenames[2])
        )
        phenotype_annotation = self._read_annotation_file(
            os.path.join(self.save_path, self.filenames[3])
        )
        all_cell_types = self._map_celltypes_to_samples(
            cell_annotation, phenotype_annotation
        )
    ...
    def _read_data_file(fpath):
        ...
        if ext == ".txt":
            ...
            sparse_matrix = (
                sparse.csr_matrix((data, (row, col)), shape=shape, dtype=np.int64)
                .transpose()
                .tocsr()
            )
        else:
    ...
    def _map_celltypes_to_samples(cell_df, samples_df):
        cell_types_per_sample = cell_df[["Main_cell_type", "sample"]].set_index(
            "sample"
        )
    ...
    def _convert_source(self, data_filepath=None):
        ...
        _ = r[
            f"write.csv(df@phenoData@data, {os.path.join(path, 'phenotype_annotation.csv')}"
        ]
        _ = r[
            f"write.csv(df@featureData@data, {os.path.join(path, 'gene_annotation.csv')}"
        ]
        _ = r[
            f"write_dgCMatrix_csv(df@assayData$exprs, {os.path.join(path, 'cleaned_data_sparse.txt')},"
    ...
    def _convert_data_to_sparse(self, fpath=None, save_path=None):
        ...
        with open(fpath, "r") as f:
            ...
            granularity_of_segments = (
                100  # nr of segments the dataset read should be split into
            )
        ...
        sparse.save_npz(
            os.path.join(save_path, "cleaned_data_sparse.npz"), sparse_matrix
        )
